...
import { Phone, Mail, Globe, ExternalLink, Copy, Check, Brain } from "lucide-react";
...
export function LeadOverviewTab({ lead, aiAnalysis, onStatusChange, saving }: Props) {
    ...
    <div className="divide-y divide-zinc-800/60">
      ...
      <div className="px-5 py-4 space-y-3">
        <div className="flex items-center justify-between">
          ...
          {aiAnalysis && (
            <span className={clsx("text-[10px] px-1.5 py-0.5 rounded border font-medium", SEVERITY_COLORS[aiAnalysis.severity])}>{aiAnalysis.severity}</span>
          )}
        </div>
        {aiAnalysis ? (
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap gap-1">
              {aiAnalysis.problems.map((p, i) => (
                <span key={i} title={p.description} className="text-[10px] bg-zinc-900 border border-zinc-800 rounded px-2 py-0.5 text-zinc-500">{p.category}</span>
              ))}
            </div>
          ...
          <p className="text-[10px] text-zinc-700">
            {lead.website_url ? 'Click "Analyze & Send" in the Outreach tab to run AI analysis.' : "No website URL — analysis not available."}
          </p>
        )}
      </div>
